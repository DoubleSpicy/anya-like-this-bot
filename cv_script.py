...
def produce_output(target_name):  
    target = cv.imread(r'./temp/target.jpg', 1)
    ...
    kernel_dilate = np.ones((9,9),np.uint8)  #bigger kernel to fill holes after ropes
    kernel_erode = np.ones((1,1),np.uint8)  #bigger kernel to delete pixels on edge that was add after dilate function
    ...
    cv.imwrite('./temp/output.jpg', res_f)
